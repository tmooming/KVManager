...
class InstanceImageUpload(Resource):
    ...
    def post(self, name):
        ...
        # 存储在本地
        image_upload_prex_dir = '/data/image-upload'
        # image_runing_prex_dir, image_file_name = "/data/image-running", "ubuntu18.04-x64-test.raw"
        # image_upload_prex_dir = image_runing_prex_dir.replace("image-running", "image-upload")
        if os.path.isfile(image_upload_prex_dir + "/" + image_file_name):
            ...
            if upload_image:
                ...
                temp['message'] = '上传就绪'
                redis_client.set(self.uuid, json.dumps(temp))
                return {"data": {"upload_md5": current_md5, "image_path": image_upload_prex_dir + "/" + image_file_name,
        ...
        if instance.get_instance_status(name) != "shut off":
            ...
            temp['message'] = '关机失败'
            redis_client.set(self.uuid, json.dumps(temp))
            history_info['action'] = name + '关机失败' if not history_info['action'] else history_info[
                                                                                          'action'] + ';' + name + '关机失败'
        ...
        temp = json.loads(redis_client.get(self.uuid))
        temp['start_time'] = copy_start_time
        temp['status'] = 'copy'
        temp['message'] = '文件拷贝中'
        redis_client.set(self.uuid, json.dumps(temp))
        try:
            ...
            if running_md5 == upload_md5:
                ...
                temp['message'] = '上传就绪'
                redis_client.set(self.uuid, json.dumps(temp))
                history_info['action'] = '文件拷贝完成' if not history_info['action'] else history_info[
                                                                                         'action'] + ';' + '文件拷贝完成'
                return {"data": {"upload_md5": upload_md5, "image_path": image_upload_prex_dir + "/" + image_file_name,
                                 'history_info': history_info}, "message": "上传就绪"}, 200
            else:
                temp = json.loads(redis_client.get(self.uuid))
                temp['start_time'] = copy_start_time
                temp['status'] = 'error'
                temp['message'] = '文件拷贝失败'
                redis_client.set(self.uuid, json.dumps(temp))
                history_info['action'] = name + '文件拷贝失败' if not history_info['action'] else history_info[
                                                                                                'action'] + ';' + name + '文件拷贝失败'
                history_info['status'] = '失败'
                db.session.add(HistoryInfo(**history_info))
                db.session.commit()
            ...
            history_info['action'] = '不存在该文件，拷贝失败' if not history_info['action'] else history_info[
                                                                                     'action'] + ';' + '不存在该文件，拷贝失败'
            history_info['status'] = '失败'
            db.session.add(HistoryInfo(**history_info))
            db.session.commit()
            return {'message':'文件拷贝失败'+str(e) },400
...
class InstanceUploadPrepareInfo(Resource):
    def get(self, uuid):
        if redis_client.get(uuid):
            ...
            if temp['status'] == 'error' and temp['step'] == 1:
                redis_client.delete(uuid)
            elif temp['status'] == 'finish' and temp['step'] == 1:
                redis_client.set(uuid,
                                 json.dumps({"start_time": time(), "step": 2, "status": "start", "message": "准备"}))
            return {"data": temp, "message": "返回成功"}, 200
        temp = {"start_time": time(), "step": 1, "status": "start", "message": "准备"}
