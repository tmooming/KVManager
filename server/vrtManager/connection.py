...
class wvmEventLoop(threading.Thread):
    """
    Event Loop Class
    启动一个daemon（守护）线程，后续的所有操作都由该线程完成，
    不影响其他服务,以单例方式启动
    """
    # _instance = None
...
class wvmConnectionManager(object):
    def __init__(self, keepalive_interval=5, keepalive_count=5):
        ...
        print('wvmConnectionManager---------------------',id(self))
...
class wvmConnect(object):
    def __init__(self, host, login, passwd, conn):
        ...
        print('wvmConnect-----------------',id(self.wvm))

    def is_qemu(self):
        return self.wvm.getURI().startswith("qemu")

    def get_hostname(self):
        return self.wvm.getHostname()
    ...
    def get_user_instances(self, name):
        ...
        return {
            "hostname": dom.name(),
            "user": '',
            "status": self.get_instance_State(dom.info()[0]),
            "uuid": dom.UUIDString(),
            "vcpu": vcpu,
            "memory": str(mem)+' MB' if mem<1024 else str(round(mem/1024,2))+" GB",
            "title": title,
            "description": description
        }
