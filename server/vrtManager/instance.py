...
class wvmInstances(wvmConnect):
    def get_instances_info(self):
        ...
        """
        instances_info = []
        instances_name = self.get_instances()
        for name in instances_name:
            instances_info.append(self.get_user_instances(name))
        return instances_info

    def get_instance_status(self, name):
        inst = self.get_instance(name)
        state, reason = inst.state()  # inst.info()[0]
        return self.get_instance_State(state)
