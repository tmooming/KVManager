...
class wvmInterfaces(wvmConnect):
    ...
    def get_ifaces_info(self):
        info = []
        for ifcace in self.get_ifaces():
            Interface = wvmInterface(self.host,self.login,self.passwd,self.conn,ifcace)
            info.append(Interface.get_details())
        return info
...
class wvmInterface(wvmConnect):
    ...
    def get_details(self):
        ...
        ip_v6 = self.get_ipv6()
        mac = self.get_mac()
        start_mode = self.get_start_mode()
        itype = self.get_type()
        state = self.is_active()
        return {"name": self.iface.name(),"IPv4":ip_v4,"IPv6":ip_v6,"start_mode":start_mode, "type": itype, "state": state, "mac": mac}
